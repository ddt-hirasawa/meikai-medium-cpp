...
public:
	...
	Matrix(int x, int y) :
		...
		ptr = new Type*[line+1];						//構築されているオブジェクト代入される ポインタが指す部分から行分
														//ポインタを作成 0 を指すとエラーが出るので要素数を1つ増やす

		*ptr = new Type[(line+1) * (col + 1)];			//構築されているオブジェクト代入される そのポインタが指す部分から
														//行×列分領域を確保 0 を指すとエラーが出るので要素数を1つ増やす

		//1から行 col までポインタを走査するためループ処理
		for (int i = 1; i <= line; i++) {

			*(ptr + i) = *ptr + (i * (col));			//今指している部分のアドレスを,列に沿ったアドレスに書き換えます
														//line は列を意味しています。col は行を意味しています。
	...
	Matrix(const Matrix& tmp) {
		...
		line = tmp.line;								//行の要素数を自分に保管します
		col = tmp.col;									//列の要素数を自分に保管します
